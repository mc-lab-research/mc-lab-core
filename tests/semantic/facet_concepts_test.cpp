...
static_assert(
    sem::TransitionRelation<const models::explicit_system&, const int&>);
static_assert(sem::TransitionLabelling<models::explicit_system,
...
static_assert(
    std::same_as<sem::initial_state_reference_t<models::explicit_system>,
                 const int&>);
static_assert(
    std::same_as<sem::transition_value_for_t<models::explicit_system, int>,
                 models::transition>);
static_assert(
    std::same_as<sem::transition_reference_for_t<models::explicit_system, int>,
                 const models::transition&>);
static_assert(std::same_as<sem::target_result_t<models::explicit_system, int>,
                           const int&>);
static_assert(
    std::same_as<sem::transition_label_for_t<models::explicit_system,
                                             const models::transition&>,
                 std::string_view>);
static_assert(
    std::same_as<sem::atomic_proposition_for_t<models::explicit_system, int>,
                 std::string_view>);
...
static_assert(
    !sem::TransitionRelation<malformed::incorrectly_qualified_target, int>);
...
static_assert(
    !sem::AtomicPropositionLabelling<malformed::non_range_propositions, int>);
static_assert(
    !sem::AtomicPropositionLabelling<malformed::adl_only_propositions, int>);
...
static_assert(
    std::is_empty_v<std::remove_cvref_t<decltype(sem::initial_states)>>);
static_assert(
    std::is_empty_v<std::remove_cvref_t<decltype(sem::outgoing_transitions)>>);
static_assert(std::is_empty_v<std::remove_cvref_t<decltype(sem::target)>>);
static_assert(
    std::is_empty_v<std::remove_cvref_t<decltype(sem::transition_label)>>);
static_assert(
    std::is_empty_v<std::remove_cvref_t<decltype(sem::atomic_propositions)>>);

static_assert(noexcept(
    sem::initial_states(std::declval<const models::explicit_system&>())));
static_assert(!noexcept(sem::initial_states(
    std::declval<const models::potentially_throwing_system&>())));
static_assert(noexcept(
    sem::outgoing_transitions(std::declval<const models::explicit_system&>(),
                              std::declval<const int&>())));
static_assert(
    noexcept(sem::target(std::declval<const models::explicit_system&>(),
                         std::declval<const models::transition&>())));
static_assert(std::same_as<decltype(sem::initial_states(
                               std::declval<const models::explicit_system&>())),
                           std::span<const int>>);
static_assert(std::same_as<decltype(sem::outgoing_transitions(
                               std::declval<const models::explicit_system&>(),
                               std::declval<const int&>())),
                           std::span<const models::transition>>);
static_assert(std::same_as<decltype(sem::target(
                               std::declval<const models::explicit_system&>(),
                               std::declval<const models::transition&>())),
                           const int&>);
static_assert(std::same_as<decltype(sem::transition_label(
                               std::declval<const models::explicit_system&>(),
                               std::declval<const models::transition&>())),
                           std::string_view>);
static_assert(std::same_as<decltype(sem::atomic_propositions(
                               std::declval<const models::explicit_system&>(),
                               std::declval<const int&>())),
                           std::span<const std::string_view>>);
static_assert(
    sem::TransitionRelation<models::potentially_throwing_relation, int>);
static_assert(!noexcept(sem::outgoing_transitions(
    std::declval<const models::potentially_throwing_relation&>(),
    std::declval<const int&>())));
static_assert(!noexcept(
    sem::target(std::declval<const models::potentially_throwing_relation&>(),
                std::declval<const models::transition&>())));
static_assert(noexcept(
    sem::transition_label(std::declval<const models::explicit_system&>(),
                          std::declval<const models::transition&>())));
static_assert(noexcept(
    sem::atomic_propositions(std::declval<const models::explicit_system&>(),
                             std::declval<const int&>())));
...
int main() {
  ...
  if (sem::target(stored, first) != 1
      || sem::transition_label(stored, first) != "advance") {
  ...
  decltype(auto) generated_outgoing =
      sem::outgoing_transitions(generated, source);
