...
static_assert(std::same_as<sem::acceptance_condition_t<models::explicit_system>,
                           condition>);
...
static_assert(std::same_as<sem::accepting_state_range_t<condition>,
                           std::span<const int>>);
static_assert(
    std::same_as<sem::accepting_state_reference_t<condition>, const int&>);

static_assert(
    !sem::HasAcceptanceCondition<malformed::missing_acceptance_condition>);
static_assert(
    !sem::HasAcceptanceCondition<malformed::void_acceptance_condition>);
static_assert(
    !sem::HasAcceptanceCondition<malformed::adl_only_acceptance_condition>);
static_assert(
    !sem::AcceptingStateSet<malformed::missing_accepting_states, int>);
static_assert(
    !sem::AcceptingStateSet<malformed::non_range_accepting_states, int>);
static_assert(
    !sem::AcceptingStateSet<malformed::incompatible_accepting_states, int>);

static_assert(
    std::is_empty_v<std::remove_cvref_t<decltype(sem::acceptance_condition)>>);
static_assert(
    std::is_empty_v<std::remove_cvref_t<decltype(sem::accepting_states)>>);
static_assert(noexcept(
    sem::acceptance_condition(std::declval<const models::explicit_system&>())));
static_assert(
    noexcept(sem::accepting_states(std::declval<const condition&>())));
static_assert(std::same_as<decltype(sem::acceptance_condition(
