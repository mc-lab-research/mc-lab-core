...
[[nodiscard]] constexpr auto outgoing_transitions(const adl_only_relation&,
                                                  const int&) noexcept {
  return std::array{adl_only_transition{}};
}

[[nodiscard]] constexpr auto target(const adl_only_relation&,
                                    const adl_only_transition&) noexcept
    -> int {
  return 0;
}
...
[[nodiscard]] constexpr auto atomic_propositions(const adl_only_propositions&,
                                                 const int&) noexcept {
...
struct non_range_accepting_states {
  [[nodiscard]] constexpr auto accepting_states() const noexcept -> int {
    return 0;
  }
};
