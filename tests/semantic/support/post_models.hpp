...
struct generated_branching_system {
  ...
  [[nodiscard]] constexpr auto post(const int& source) const {
    return std::views::iota(1, 5)
           | std::views::filter([source](const int candidate) {
               switch (source) {
               case 0:
                 return candidate <= 2;
               case 1:
...
struct non_range_post {
  [[nodiscard]] constexpr auto post(const int&) const noexcept -> int {
    return 0;
  }
};
...
struct incompatible_post {
  [[nodiscard]] constexpr auto post(const int&) const noexcept {
    return std::array<short, 1>{0};
  }
};

/** A non-const member cannot satisfy the logical-const observation contract. */
struct non_const_post {
  [[nodiscard]] constexpr auto post(const int& state) noexcept {
    return std::array{state};
  }
};
...
[[nodiscard]] constexpr auto post(const adl_only_post&,
                                  const int& state) noexcept {
  return std::array{state};
}
