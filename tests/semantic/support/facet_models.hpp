...
struct accepting_state_condition {
  ...
  [[nodiscard]] constexpr auto accepting_states() const noexcept
      -> std::span<const int> {
    return states;
  }
};
...
struct explicit_system {
  ...
  [[nodiscard]] constexpr auto acceptance_condition() const noexcept
      -> const accepting_state_condition& {
    return acceptance;
  }
  ...
  [[nodiscard]] constexpr auto outgoing_transitions(const int&) const noexcept
      -> std::span<const transition> {
    return outgoing;
  }

  /** The target is a reference into the stored transition witness. */
  [[nodiscard]] constexpr auto target(const transition& edge) const noexcept
      -> const int& {
  ...
  [[nodiscard]] constexpr auto atomic_propositions(const int&) const noexcept
...
struct potentially_throwing_relation {
  ...
  [[nodiscard]] auto outgoing_transitions(const int&) const
      -> std::span<const transition> {
    return outgoing;
  }

  [[nodiscard]] auto target(const transition& edge) const -> int {
    return edge.destination;
  }
};
