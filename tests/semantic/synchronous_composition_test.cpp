...
struct blocked_system {
  [[nodiscard]] constexpr auto initial_states() const noexcept {
    return std::array{0};
  }
  ...
  [[nodiscard]] constexpr auto target(const int transition) const noexcept
      -> int {
    return transition;
  }
};
...
concept CanSynchronousCompose = requires(Systems&&... systems) {
  sem::synchronous_compose(std::forward<Systems>(systems)...);
};
...
static_assert(
    std::same_as<typename composition::transition_type, product_transition>);
static_assert(
    std::same_as<typename heterogeneous_composition::transition_type,
...
static_assert(
    std::same_as<sem::transition_value_for_t<composition, product_state>,
                 product_transition>);
...
int run_tests() {
  ...
  for (const auto& transition : outgoing) {
    ...
    const auto second_index =
        static_cast<std::size_t>(std::get<1>(destination));
  ...
  if (!sem::outgoing_transitions(blocked_product, product_state{0, 0, 0})
           .empty()) {
