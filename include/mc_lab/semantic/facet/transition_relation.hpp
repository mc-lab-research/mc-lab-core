...
concept OutgoingTransitionsCallable =
    requires(const std::remove_cvref_t<System>& system,
             const std::remove_cvref_t<State>& state) {
      outgoing_transitions(system, state);
    };

template <class System, class State>
using outgoing_transition_range_t = decltype(outgoing_transitions(
    std::declval<const std::remove_cvref_t<System>&>(),
    std::declval<const std::remove_cvref_t<State>&>()));

template <class System, class State>
concept OutgoingTransitionsInputRange =
    OutgoingTransitionsCallable<System, State>
...
// required.
template <class System, class State>
concept TargetCallable =
    OutgoingTransitionsInputRange<System, State>
    && requires(const std::remove_cvref_t<System>& system,
                transition_reference_for_t<System, State> transition) {
         target(system, std::forward<transition_reference_for_t<System, State>>(
                            transition));
       };

template <class System, class State>
requires TargetCallable<System, State>
using target_result_t =
    decltype(target(std::declval<const std::remove_cvref_t<System>&>(),
...
template <class System, class State>
concept StateCompatibleTarget =
    TargetCallable<System, State>
