...
template <class System, class State>
using atomic_proposition_range_t = decltype(atomic_propositions(
    std::declval<const std::remove_cvref_t<System>&>(),
    std::declval<const std::remove_cvref_t<State>&>()));

template <class System, class State>
concept AtomicPropositionsInputRange =
    AtomicPropositionsCallable<System, State>
...
using atomic_proposition_range_t =
    detail::atomic_proposition_range_t<System, State>;
 ...
 */
template <class System, class State>
concept AtomicPropositionLabelling =
    detail::AtomicPropositionsInputRange<System, State>;
