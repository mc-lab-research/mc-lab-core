...
concept InitialStatesCallable = requires(
    const std::remove_cvref_t<System>& system) { initial_states(system); };

template <class System>
using initial_state_range_t = decltype(initial_states(
...
concept InitialStatesInputRange =
    InitialStatesCallable<System>
    && std::ranges::input_range<initial_state_range_t<System>>;
...
using initial_state_reference_t =
    std::ranges::range_reference_t<initial_state_range_t<System>>;
