...
concept PostCallable =
    requires(const std::remove_cvref_t<System>& system,
             const std::remove_cvref_t<State>& state) { post(system, state); };

template <class System, class State>
using post_range_for_t =
    decltype(post(std::declval<const std::remove_cvref_t<System>&>(),
                  std::declval<const std::remove_cvref_t<State>&>()));

template <class System, class State>
concept PostInputRange =
    PostCallable<System, State>
    && std::ranges::input_range<post_range_for_t<System, State>>;

template <class System, class State>
requires PostInputRange<System, State>
using post_reference_for_t =
    std::ranges::range_reference_t<post_range_for_t<System, State>>;

template <class System, class State>
concept PostStateCompatible =
    PostInputRange<System, State>
    && std::same_as<std::remove_cvref_t<std::ranges::range_value_t<
