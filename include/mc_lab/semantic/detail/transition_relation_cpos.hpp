...
struct outgoing_transitions_fn {
  ...
  requires requires(System&& system, State&& state) {
    std::forward<System>(system).outgoing_transitions(
        std::forward<State>(state));
  }
  [[nodiscard]] constexpr decltype(auto) operator()(System&& system,
                                                    State&& state) const
      noexcept(noexcept(std::forward<System>(system).outgoing_transitions(
          std::forward<State>(state)))) {
    return std::forward<System>(system).outgoing_transitions(
        std::forward<State>(state));
  }
};
