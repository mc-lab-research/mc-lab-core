...
using synchronous_transition_t =
    transition_value_for_t<std::remove_cvref_t<System>,
                           synchronous_state_t<System>>;

template <class System>
using synchronous_transition_reference_t =
    transition_reference_for_t<std::remove_cvref_t<System>,
                               synchronous_state_t<System>>;

template <class System>
concept StableSynchronousOperand =
    InitialStateSet<std::remove_cvref_t<System>>
    && TransitionRelation<std::remove_cvref_t<System>,
                          synchronous_state_t<System>>
    ...
    && requires(const std::remove_cvref_t<System>& system,
         ...
         requires std::same_as<
             std::remove_cvref_t<decltype(mc_lab::semantic::target(
                 system, transition))>,
             synchronous_state_t<System>>;
       };
...
requires(sizeof...(Systems) >= 2)
        && (detail::StableSynchronousOperand<Systems> && ...)
class synchronous_composition_view {
public:
  using state_type = std::tuple<detail::synchronous_state_t<Systems>...>;
  using transition_type =
      std::tuple<detail::synchronous_transition_t<Systems>...>;

  constexpr explicit synchronous_composition_view(
  ...
  [[nodiscard]] auto target(const transition_type& transition) const
      -> state_type {
...
private:
  ...
  void append_initial_states(Prefix prefix,
                             std::vector<state_type>& result) const {
  ...
  void append_transitions(const state_type& source, Prefix prefix,
    ...
    if constexpr (Index == sizeof...(Systems)) {
      ...
      auto transitions = mc_lab::semantic::outgoing_transitions(
          system<Index>(), std::get<Index>(source));
      using component_transition_type =
  ...
  template <std::size_t... Index>
  [[nodiscard]] auto target_impl(const transition_type& transition,
                                 std::index_sequence<Index...>) const
      -> state_type {
    return state_type{mc_lab::semantic::target(system<Index>(),
                                               std::get<Index>(transition))...};
  }
...
[[nodiscard]] constexpr auto synchronous_compose(Systems&... systems) {
  return synchronous_composition_view<std::remove_cvref_t<Systems>...>{
      systems...};
}
