...
concept AcceptingStatesCallable =
    requires(const std::remove_cvref_t<Condition>& condition) {
      accepting_states(condition);
    };

template <class Condition>
using accepting_state_range_t = decltype(accepting_states(
    std::declval<const std::remove_cvref_t<Condition>&>()));

template <class Condition>
concept AcceptingStatesInputRange =
    AcceptingStatesCallable<Condition>
...
concept AcceptingStatesCompatible =
    ...
    && std::same_as<std::remove_cvref_t<std::ranges::range_value_t<
                        accepting_state_range_t<Condition>>>,
