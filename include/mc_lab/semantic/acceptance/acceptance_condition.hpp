...
using acceptance_condition_result_t = decltype(acceptance_condition(
    std::declval<const std::remove_cvref_t<System>&>()));

template <class System>
concept AcceptanceConditionObject =
    AcceptanceConditionCallable<System>
    && std::is_object_v<
        std::remove_cvref_t<acceptance_condition_result_t<System>>>;
...
using acceptance_condition_t =
    std::remove_cvref_t<detail::acceptance_condition_result_t<System>>;
